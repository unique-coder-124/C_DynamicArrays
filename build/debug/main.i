...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 100; i++) {
    ({ if (test.count >= test.capacity) { if (test.capacity == 0) { test.capacity = 128; } else { test.capacity *= 2; } test.items = realloc(test.items, test.capacity*sizeof(*test.items)); } test.items[test.count++] = i; });
  }
